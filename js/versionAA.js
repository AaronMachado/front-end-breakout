//Good Idea: Make the sense of the X bouncing random (between -1 and 1 excluding 0) when
...
var brick_counter = 1;//We already have one in the html
...
/********************Wall*******************/
	...
	var firstBrick = document.createElement("div"); //Constructor (In java speaking)
		...
		firstBrick.style.width = 80 + "px"; //Assigns the new "responsive" value.
	/**/

	/**/
	var windowWidth = window.innerWidth;
	var           a = Math.round(windowWidth/firstBrick.clientWidth);
	var 		  b = windowWidth/a;
	
	var 		  c = Math.floor( windowWidth / (Math.round(windowWidth/firstBrick.clientWidth)) );
	console.log("The window Width is "+windowWidth+" And Width/brickWidth is "+ a);
	console.log("The value of b is "+b+" And the round down of b is "+Math.floor(b) );
	...
	var numberOfBricks = a;
	var margin = (windowWidth-(numberOfBricks*brickWidth) ) / 2;


	var separation = 0;
	
	var brick_x = getPosition(firstBrick).x + brickWidth + separation; //Because we already have the first one on html
...
/****************First Layer****************/
	function moveBall(){
		...
		RAF_ID = window.requestAnimationFrame(moveBall);
...
/****************Second Layer****************/
	...
	function checkTopAndBottom(){
		...
		class_of_the_element = "notBrick";/*What is this shit doing? find out or remove*/
